...
# meshctl stores its session itself; MESH_CONFIG puts it in the control plane's volume so it
# survives the container being recreated. Nothing here has to carry a token around.
cpctl() {
    docker exec -e MESH_CP_URL=http://meshcp:8080 -e MESH_CONFIG=/var/lib/meshcp/cli.json \
        meshcp /opt/mesh/meshctl "$@"
}
OUT=$(cpctl signup "$EMAIL" "$PASSWORD" "$SUBNET" 2>&1) || OUT=$(cpctl login "$EMAIL" "$PASSWORD")
echo "$OUT" | grep -q '^account ' || { echo "could not log in:"; echo "$OUT"; exit 1; }
echo "logged in"
